...
function OpinionCard({ data, isActive }) {
  return (
    <div
      className={`transition-opacity duration-500 absolute top-14 sm:top-0 left-0 right-0 bottom-0 ${
        data.id === isActive ? "opacity-100" : "opacity-0 pointer-events-none"
      }`}
    >
      <div className="bg-gray-200 transform hover:scale-105 transition-all duration-500 p-3 rounded-lg shadow-md w-[300px] mx-auto">
